 ...
 *
 * 22 Aug 2009  0.6  Correct checking of the convert to the nearest limit
...
static void do_set_control_value(__u32 id, __s32 value, struct v4l2_queryctrl *queryctrl) {
	...
	if (ret_set == -1) {
		...
		if (ret_get == 0) {
			...
			else if (queryctrl->maximum <= value)
				CU_ASSERT_EQUAL(control_new.value, queryctrl->maximum)
			else
				CU_ASSERT_EQUAL(control_new.value, value);
		}
	}
}
