 ...
 *
 * 23 Aug 2009  0.4  Allow EFAULT and ENOMEM error reporting when the count
 *                   value is invaild
 * 23 Aug 2009  0.3  Correct typo
 * 22 Aug 2009  0.2  Fix typo in comment
...
void test_VIDIOC_G_EXT_CTRLS_zero_invalid_count() {
	...
	dprintf("\t%s:%u: VIDIOC_G_EXT_CTRLS, count=0x%x, ret_get=%i, errno_get=%i\n",
	...
	do {
		...
		if (ret_get == 0) {
			...
			CU_ASSERT_EQUAL(ret_get_invalid, -1);
			CU_ASSERT(errno_get_invalid == EINVAL || errno_get_invalid == EFAULT || errno_get_invalid == ENOMEM);
		}
		count <<= 1;
	} while (count != 0);
	...
	if (ret_query == 0) {
		...
		case V4L2_CTRL_TYPE_MENU:
			if (ret_get == 0) {
				...
				/* This is the case when VIDIOC_G_EXT_CTRLS is not
				 * supported at all.
				 */
				CU_ASSERT_EQUAL(ret_get, -1);
				CU_ASSERT_EQUAL(errno_get, EINVAL);
...
void test_VIDIOC_S_EXT_CTRLS_zero_invalid_count() {
	...
	dprintf("\t%s:%u: VIDIOC_S_EXT_CTRLS, count=0x%x, ret_set=%i, errno_set=%i\n",
	...
	do {
		...
		if (ret_set == 0) {
			...
			CU_ASSERT_EQUAL(ret_set_invalid, -1);
			CU_ASSERT(errno_set_invalid == EINVAL || errno_set_invalid == EFAULT || errno_set_invalid == ENOMEM);
		}
		count <<= 1;
	} while (count != 0);
...
void test_VIDIOC_TRY_EXT_CTRLS_zero_invalid_count() {
	...
	do {
		...
		if (ret_try == 0) {
			...
			CU_ASSERT_EQUAL(ret_try_invalid, -1);
			CU_ASSERT(errno_try_invalid == EINVAL || errno_try_invalid == EFAULT || errno_try_invalid == ENOMEM);
		}
		count <<= 1;
	} while (count != 0);
